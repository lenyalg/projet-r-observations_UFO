...
p <- ufo_data |>
      ...
      labs(x = "Année", y = "Nombre d'observations", title = "Nombre d'observations par Année")
...
p1 <- ufo_data |>
      ...
      theme_minimal(base_size = 12) +
      labs(x = "Mois", y = "Nombre d'observations", title = "Nombre d'observations par Mois")
    g1 <- ggplotly(p1) |> config(displayModeBar = FALSE)
...
p2 <- ufo_data |>
      ...
      labs(x = "Heure", y = "Nombre d'observations", title = "Nombre d'observations par Heure")
    g2 <- ggplotly(p2) |> config(displayModeBar = FALSE)
    
# Combinez les graphiques
combined_plot <- subplot(g1, g2, nrows = 1, margin = 0.05, shareY = FALSE) |>
  layout(
    title = list(text = ""),
    
    xaxis = list(title = "Mois"), 
    xaxis2 = list(title = "Heure"), 
    
    yaxis = list(title = "Nombre d'observations"), 
    yaxis2 = list(title = "Nombre d'observations"), 
    
    annotations = list(
      list(x = 0, y = 1.05, text = "Observations par Mois",
           xref = "paper", yref = "paper", showarrow = FALSE, font = list(size = 18)),
      list(x = 0.75, y = 1.05, text = "Observations par Heure",
           xref = "paper", yref = "paper", showarrow = FALSE, font = list(size = 18))
    )
  )
...
```{r heatmap, echo=FALSE, message = FALSE, warning=FALSE, out.width='100%', fig.width=12, fig.height=4 }
    ...
    p <- ggplot(heatmap_data, aes(x = hour_val, y = wday_label, fill = n)) +
      ...
      theme_minimal(base_size = 12) +
      labs(x = "Heure (00h-23h)", y = "Jour", fill = "Nombre d'observations", title = "Heatmap des Jour/Heure") +
...
p <- ufo_data |>
      ...
      coord_flip() +
      theme_minimal(base_size = 12) +
      labs(x = "Forme", y = "Nombre d'observations", title = "Nombre d'observations par forme")
